...
public class XMLConfigBuilder extends BaseBuilder {
  ...
  public XMLConfigBuilder(InputStream inputStream, String environment, Properties props) {
    // XPathParaser基于 Java XPath 解析器，用于解析 MyBatis 中的配置文件
    this(new XPathParser(inputStream, true, props, new XMLMapperEntityResolver()), environment, props);
  }

  private XMLConfigBuilder(XPathParser parser, String environment, Properties props) {
    // 创建 Configuration 对象，并通过 TypeAliasRegistry 注册一些 Mybatis 内部相关类的别名
  ...
  private void parseConfiguration(XNode root) {
    try {
      ...
      // 解析 </typeAliases> 标签【类型别名】
      typeAliasesElement(root.evalNode("typeAliases"));
      // 解析 </plugins> 标签【插件】
      pluginElement(root.evalNode("plugins"));
      // 解析 </objectFactory> 标签【对象工厂】
      objectFactoryElement(root.evalNode("objectFactory"));
      // 解析 </objectWrapperFactory> 标签【对象封装工厂】
      objectWrapperFactoryElement(root.evalNode("objectWrapperFactory"));
      // 解析 </reflectorFactory> 标签【反射工厂】
      reflectorFactoryElement(root.evalNode("reflectorFactory"));
      settingsElement(settings);
      // read it after objectFactory and objectWrapperFactory issue #631
      // 解析 </environments> 标签【环境配置】
      environmentsElement(root.evalNode("environments"));
      // 解析 </databaseIdProvider> 标签【数据库厂商标识】
      databaseIdProviderElement(root.evalNode("databaseIdProvider"));
      // 解析 </typeHandlers> 标签【类型处理器】
      typeHandlerElement(root.evalNode("typeHandlers"));
      // 解析 </mappers> 标签【映射器】加载映射文件流程主入口
    ...
    if (context != null) {
      ...
      for (XNode child : context.getChildren()) {
        ...
        if (isSpecifiedEnvironment(id)) {
          ...
          // 将 environment 存到 configuration 中
      ...
      for (XNode child : parent.getChildren()) {
        ...
        if ("package".equals(child.getName())) {
          ...
          if (resource != null && url == null && mapperClass == null) {
            ...
            // 将指定 mapper 接口以及它的代理存储到一个 Map 集合中，key 为 mapper 接口类型，value 为代理对象工厂
