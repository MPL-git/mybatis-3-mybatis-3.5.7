...
public class Configuration {
  ...
  // 是否开启自动驼峰命名规则（camel case）映射，既从经典数据库列名 A_COLUMN
  // 到经典 Java 属性名 aColum 的类似映射。默认 false
  protected boolean mapUnderscoreToCamelCase;
  // 当开启时，任何方法的调用都会加载该对象的所有属性。否则，每个属性会按需加载。默认值 false（true in ≤ 3.4.1）
  protected boolean aggressiveLazyLoading;
  // 是否允许单一语句返回多结果集（需要兼容驱动）
  protected boolean multipleResultSetsEnabled = true;
  // 允许 JDBC 支持自动生成主键，需要驱动兼容。这就是 insert 时获取 mysql 自增主键 / oracle sequence 的开关
  // 注：一般来说，这是希望的结果，应该默认值为 true 比较合适。
  protected boolean useGeneratedKeys;
  // 使用列标签代替列明，一般来说，这是希望的结果
  ...
  public StatementHandler newStatementHandler(Executor executor, MappedStatement mappedStatement, Object parameterObject, RowBounds rowBounds, ResultHandler resultHandler, BoundSql boundSql) {
    // 创建路由功能的 statementHandler，根据 MappedStatement 中的 StatementType
